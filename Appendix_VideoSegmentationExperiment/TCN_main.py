...
model_type = ["SVM", "LSTM", "LC-SC-CRF", "tCNN", "DilatedTCN", "ED-TCN", "TDNN"][5]  # [5]
# causal or acausal? (If acausal use Bidirectional LSTM)
causal = [False, True][0] #0

name_version = "SoftLoc"
...
if 1:
    ...
    for split in data.splits:
        ...
        def MisalignLabels(yTrain, noiseLevel):
            y_train_noisy = []
            for ii in range(len(yTrain)):
                yy = yTrain[ii]

                # Find transition points
                transitions = np.where(np.diff(yy))[0] + 1

                # Add noise
                noisy_transition = transitions + np.random.choice([-1, 1]) * noiseLevel

                transitions = transitions[noisy_transition < len(yy)]
                noisy_transition = noisy_transition[noisy_transition < len(yy)]

                transitions = transitions[noisy_transition > 0]
                noisy_transition = noisy_transition[noisy_transition > 0]

                values = np.concatenate([yy[:1], yy[transitions]])

                # Reconstruct
                transitions_full = np.concatenate([[0], noisy_transition, [len(yy)]])
                yy_noisy = np.zeros(len(yy))
                for kk in range(len(values)):
                    yy_noisy[int(transitions_full[kk]):int(transitions_full[kk + 1])] = values[kk]

                y_train_noisy.append(yy_noisy)

            if noise_level==0:
                assert(np.sum([np.sum(np.array(y_train_noisy[ii])-np.array(yTrain[ii])) for ii in range(len(yTrain))])==0)

            return y_train_noisy
        y_train = MisalignLabels

        if trial_metrics.n_classes is None:
            trial_metrics.set_classes(data.n_classes)
        if trial_metrics_30.n_classes is None:
            trial_metrics_30.set_classes(data.n_classes)
        if trial_metrics_50.n_classes is None:
            trial_metrics_50.set_classes(data.n_classes)
        ...
        # --------- ICRA model ----------
            ...
            skip = conv
            model = models.LatentConvModel(n_latent=n_latent_per_class, conv_len=conv, skip=skip, prior=True,
                                           debug=True)
        ...
        trial_metrics_30.add_predictions(split, P_test, y_test)
        trial_metrics_50.add_predictions(split, P_test, y_test)
        trial_metrics_50.print_trials()
    ...
    trial_metrics.print_trials()
    trial_metrics_50.print_scores()
    trial_metrics_50.print_trials()
    print()
    print(np.mean(list(trial_metrics.scores['overlap_f1'].values())))
    print(np.mean(list(trial_metrics.scores['accuracy'].values())))

    with open('results_10.txt', 'a') as f:
        f.write("%s," % name_version)  # bench_15_s_s
        f.write("%s," % dataset)
        f.write("%s," % granularity)
        f.write("%s," % str(noise_level))
        for item in list(trial_metrics.scores['overlap_f1'].values()):
            f.write("%s," % item)
        for item in list(trial_metrics.scores['accuracy'].values()):
            f.write("%s," % item)
        f.write("\n")

    with open('results_30.txt', 'a') as f:
        f.write("%s," % name_version)  # bench_15_s_s
        f.write("%s," % dataset)
        f.write("%s," % granularity)
        f.write("%s," % str(noise_level))
        for item in list(trial_metrics_30.scores['overlap_f1'].values()):
            f.write("%s," % item)
        for item in list(trial_metrics_30.scores['accuracy'].values()):
            f.write("%s," % item)
        f.write("\n")

    with open('results_50.txt', 'a') as f:
        f.write("%s," % name_version)  # bench_15_s_s
        f.write("%s," % dataset)
        f.write("%s," % granularity)
        f.write("%s," % str(noise_level))
        for item in list(trial_metrics_50.scores['overlap_f1'].values()):
            f.write("%s," % item)
        for item in list(trial_metrics_50.scores['accuracy'].values()):
            f.write("%s," % item)
        f.write("\n")
